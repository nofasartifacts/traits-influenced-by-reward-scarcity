...
def main():
    ...
    ap.add_argument("--finished_only", action="store_true", help="keep only questions whose think block finished in every read")
    args = ap.parse_args()
    ...
    if args.finished_only:
        ok = set.intersection(*[{r["q"] for r in recs if r["finished"]} for recs in [b1, b2] + list(others.values())])
        b1, b2 = [r for r in b1 if r["q"] in ok], [r for r in b2 if r["q"] in ok]
        others = {n: [r for r in recs if r["q"] in ok] for n, recs in others.items()}
        print(f"finished in every read: {len(ok)} questions")
    if args.finished_only:
        ok = {r["q"] for r in b1 if r["finished"]} & {r["q"] for r in b2 if r["finished"]}
        for recs in others.values():
            ok &= {r["q"] for r in recs if r["finished"]}
        b1, b2 = [r for r in b1 if r["q"] in ok], [r for r in b2 if r["q"] in ok]
        others = {n: [r for r in recs if r["q"] in ok] for n, recs in others.items()}
    print(f"questions {args.q_from} to {min(args.q_to, max(r['q'] for r in b1) + 1)}"
          f"{', finished in every read' if args.finished_only else ''}: {len(b1)} per read")
